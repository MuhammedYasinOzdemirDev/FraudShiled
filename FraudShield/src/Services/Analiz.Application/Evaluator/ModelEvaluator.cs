       ...
       public async Task<ModelMetrics> EvaluateAsync(ITransformer model, IDataView evaluationData)
{
    try
    {
        ...
        int positiveCount = predictionEnumerable.Count(p => p.Label == true);
        ...
        if (positiveCount == 0)
        {
            ...
            int truePositives = actualLabels.Zip(predictedLabels, (a, p) => a && p).Count(x => x);
            int falsePositives = actualLabels.Zip(predictedLabels, (a, p) => !a && p).Count(x => x);
            int trueNegatives = actualLabels.Zip(predictedLabels, (a, p) => !a && !p).Count(x => x);
            int falseNegatives = actualLabels.Zip(predictedLabels, (a, p) => a && !p).Count(x => x);

            double accuracy = predictedLabels.Zip(actualLabels, (p, a) => p == a).Count(x => x) / (double)actualLabels.Count;
            double precision = (truePositives + falsePositives) > 0 ? truePositives / (double)(truePositives + falsePositives) : 0;
            double recall = (truePositives + falseNegatives) > 0 ? truePositives / (double)(truePositives + falseNegatives) : 0;
        ...
        else
        {
            ...
            {
                _logger.LogWarning("BinaryClassification evaluator could not compute AUC due to insufficient positive samples. Computing manual metrics.");
