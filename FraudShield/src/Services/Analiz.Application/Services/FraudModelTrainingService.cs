...
public class FraudModelTrainingService : IFraudModelTrainingService
{
    ...
    public async Task<(TrainingResult LightGBM, TrainingResult PCA)> TrainModelsAsync()
    {
        try
        {
            ...
            var lightGBMRequest = await CreditCardModelConfiguration.CreateTrainingRequest(data, ModelType.LightGBM,_extractionService, _logger);
            ...
            // PCA modeli eğitimi
            var pcaRequest =await CreditCardModelConfiguration.CreateTrainingRequest(data, ModelType.PCA, _extractionService,_logger);
    ...
    public async Task<TrainingResult> TrainEnsembleModelAsync()
    {
        try
        {
            ...
            var ensembleRequest = await CreditCardModelConfiguration.CreateEnsembleTrainingRequest(data,_extractionService, _logger);
