...
public class FeatureEngineeringService : IFeatureExtractionService
{
    ...
    public async Task<bool> UpdateFeatureConfigurationAsync(FeatureConfig config)
    {
        try
        {
            ...
            {
                currentConfig = await CreateAndSaveDefaultConfiguration();
            }

            currentConfig.UpdateFeatures(config.EnabledFeatures);
            currentConfig.UpdateSettings(config.FeatureSettings);

            await _configRepository.UpdateAsync(currentConfig);
            return true;
        }
        catch (Exception ex)
        {
        ...
        ModelType modelType)
    {
        ...
        if (config.EnabledFeatures.GetValueOrDefault("TimeFeatures", true))
        {
            ...
            foreach (var feature in timeFeatures)
            {
        ...
        if (config.EnabledFeatures.GetValueOrDefault("TransactionFeatures", true))
        {
            ...
            foreach (var feature in transactionFeatures)
            {
                features[feature.Key] = feature.Value;
            }
        }
        ...
        if (config.EnabledFeatures.GetValueOrDefault("PCAFeatures", true))
        {
            ...
            foreach (var feature in pcaFeatures)
            {
                features[feature.Key] = feature.Value;
            }
        }
    ...
    private async Task<FeatureConfiguration> CreateAndSaveDefaultConfiguration()
    {
        ...
        await _configRepository.UpdateAsync(defaultConfig);
    ...
    private FeatureSetting CreateTimeSetting(string name) => new()
    {
        Name = name,
    ...
    private FeatureSetting CreateTransactionSetting(string name, string transform) => new()
    {
        Name = name,
        Type = FeatureType.Numeric,
    ...
    private FeatureSetting CreatePCASetting(string name, string transform) => new()
    {
        Name = name,
        Type = FeatureType.Numeric,
        ...
        try
        {
            ...
            foreach (var (feature, stats) in statistics)
            {
                var importance = FeatureImportance.Create(
                    ...
                    featureName: feature,
                    importance: stats["variance"],
                    category: DetermineFeatureCategory(feature),
                    statistics: stats);
    ...
    private Dictionary<string, float> ExtractTimeFeatures(DateTime timestamp)
    {
        ...
        double timeOfDay = timestamp.TimeOfDay.TotalSeconds;
        ...
        Dictionary<string, double> normParams)
    {
        ...
        float normalizedAmount = NormalizeAmount(amountFloat, normParams);
    ...
    private Dictionary<string, float> ExtractPCAFeatures(TransactionData data)
    {
        ...
        if (data.AdditionalData != null)
        {
            ...
            {
                string key = $"V{i}";
                if (data.AdditionalData.TryGetValue(key, out var value) &&
                    float.TryParse(value, out float floatValue))
                {
                    features[key] = floatValue;
                    features[$"{key}_normalized"] = NormalizeFeature(floatValue);
                }
            }
        }
    ...
    {
        if (normParams.TryGetValue("AmountMin", out double min) &&
            normParams.TryGetValue("AmountMax", out double max))
        {
