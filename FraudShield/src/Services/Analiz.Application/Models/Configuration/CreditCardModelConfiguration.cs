...
public class CreditCardModelConfiguration
{
        ...
        ILogger logger)
    {
        try
        {
            var mlContext = new MLContext(seed: RANDOM_SEED);
            ...
            var featureConfig = new FeatureConfig
            {
                ...
                FeatureSettings = new Dictionary<string, FeatureSetting>
                {
                    ...
                    ["HourFeature"] = new FeatureSetting
                    {
                        Name = "HourFeature",
                        Type = FeatureType.Categorical,
                        IsRequired = true,
                    ...
                    ["Amount_normalized"] = new FeatureSetting
                    {
    ...
    public static EnsembleConfiguration CreateEnsembleConfig(IDataView trainData, MLContext mlContext, ILogger logger)
    {
        ...
        var pcaConfig = CreateAnomalyDetectionConfig(mlContext.Data
            .CreateEnumerable<CreditCardModelData>(trainData, reuseRowObject: false)
            .ToList());
    ...
    private static Dictionary<string, string> CreateAdditionalData(CreditCardModelData data)
    {
        ...
        for (int i = 1; i <= 28; i++)
        {
            var property = typeof(CreditCardModelData).GetProperty($"V{i}");
            if (property != null)
            {
                var value = property.GetValue(data);
                additionalData[$"V{i}"] = value?.ToString();
            }
        }

        // Zaman özelliğini ekle
        additionalData["Time"] = data.Time.ToString();
    ...
    private static IDataView BalanceDataWithUndersampling(MLContext mlContext, IDataView data, ILogger logger)
    {
        ...
        {
            logger.LogInformation($"Column {col.Name}: {col.Type}");
        }
    ...
    {
        var records = mlContext.Data.CreateEnumerable<CreditCardModelData>(data, reuseRowObject: false).ToList();
        var random = new Random(RANDOM_SEED);

        var fraudRecords = records.Where(x => x.Label).ToList();
        var nonFraudRecords = records.Where(x => !x.Label).ToList();

        int fraudTestCount = (int)(fraudRecords.Count * testFraction);
        int nonFraudTestCount = (int)(nonFraudRecords.Count * testFraction);
    ...
    private static List<string> GetEnhancedFeatureColumns()
    {
        ...
        for (int i = 1; i <= 28; i++)
        {
            baseFeatures.Add($"V{i}");
        }

        // V1-V28 normalize edilmiş özellikler
        for (int i = 1; i <= 28; i++)
        {
            baseFeatures.Add($"V{i}_normalized");
        }
